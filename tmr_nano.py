...
for line in RELS:
    ...
    zOTU_match = re.search(r"^Zotu\d+", LINE[0])
    if zOTU_match:
        zOTU = zOTU_match[0]
        ...
        print('FATAL ERROR! No valid Zotu ID found in line:', LINE[0])
    elif  LINE[1] == "noisy_chimera" or LINE[1] == "perfect_chimera" or LINE[1] == "match_chimera" or "Chimera" in LINE[2]:
...
for row_no in range(0, len(OTU_TABLE)):
    if OTU_TABLE[row_no][1] in otu_dict and otu_dict[OTU_TABLE[row_no][1]][-1] != 1:
